...
@section('title', 'SewaLux - Success Booking')
@section('content')
    ...
    <main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
        <section id="finishBook" class="flex flex-col gap-[30px] max-w-[353px] p-[30px_20px] items-center m-auto">
            <div class="flex flex-col gap-2 items-center">
                ...
                <p class="leading-[30px] text-[#6E6E70] text-center">Kami akan segera menghubungi anda untuk proses pemberian
                    barang</p>
            </div>
            <div class="w-full h-[220px] flex items-center justify-center flex-shrink-0">
                <img src="{{ Storage::url($transaction->product->thumbnail) }}" alt="Thumbnail"
            ...
            <div class="w-[220px] flex flex-col gap-3 items-center">
                <a href="{{ route('front.index') }}"
                    class="w-full text-center rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold text-black">Rent More</a>
                <a href="{{ route('front.transactions') }}"
                    class="w-full text-center rounded-full p-[12px_24px] bg-white font-bold text-black outline outline-1">Booking
                    Details</a>
            </div>
        </section>
    </main>
